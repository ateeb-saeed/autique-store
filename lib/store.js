...
const FILES = {
  ...
  logistics: path.join(DATA_DIR, 'logistics.json'),
  stockLog: path.join(DATA_DIR, 'stock-log.json'),
  payments: path.join(DATA_DIR, 'payments.json')
};

const DEFAULT_ADMIN_PASSWORD = 'autique-admin';
const DEFAULT_LOGISTICS_PASSWORD = 'autique-logistics';
...
function seed() {
  ...
  if (!fs.existsSync(FILES.stockLog)) writeJSON(FILES.stockLog, []);
  ...
  if (!fs.existsSync(FILES.logistics)) {
    writeJSON(FILES.logistics, {
      password_hash: bcrypt.hashSync(DEFAULT_LOGISTICS_PASSWORD, 10)
    });
    console.log(`\nLogistics portal created. Sign in at /logistics.html with this password: ${DEFAULT_LOGISTICS_PASSWORD}`);
    console.log('The admin can change it under Admin settings.\n');
  }
...
const store = {
  ...
  getLogistics: () => readJSON(FILES.logistics, null),
  saveLogistics: (obj) => writeJSON(FILES.logistics, obj),

  getStockLog: () => readJSON(FILES.stockLog, []),
  saveStockLog: (list) => writeJSON(FILES.stockLog, list),

  getPayments: () => readJSON(FILES.payments, []),
  savePayments: (list) => writeJSON(FILES.payments, list),
