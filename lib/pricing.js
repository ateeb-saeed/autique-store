...
// Pass the order subtotal to also check the coupon's minimum order.
function couponStatus(coupon, subtotal) {
  ...
  if (subtotal !== undefined && coupon.minOrder && subtotal < coupon.minOrder) {
    return { valid: false, reason: `That code needs an order of at least Rs. ${coupon.minOrder}.` };
  }
  return { valid: true };
}
